...
Default to injecting the import map through the build's HTML transform — the host's `index.html` stays hand-authored and untouched in source; the build writes a transformed copy with the map inlined into its own output directory, never mutating the source file. It costs nothing extra: no additional network request, no separate script to parse and execute before the map takes effect. Reach for a standalone-asset delivery instead only when you have a concrete reason to: a dev server proxying to a shared manifest, a host that isn't built with the same tool that generates the map, or a team that wants one import-map endpoint several local dev servers can point at.
...
A host that's a purely static page — no bundled app JS of its own, just the HTML shell, the import map, and static assets like images or stylesheets — still builds through the same tool. Vite's own convention for this is a `public/` directory: anything placed there is copied to the same relative path under the build output verbatim, with no processing or renaming, which is exactly "this directory becomes the literal domain root once deployed." No custom copy step needed for it.
